...
async function sendToUserTestNumber() {
  const targetPhone = '9136870930';
  const templateName = 'universal_b2b_web_v2';
  const params = ['Pink Octopus by T & J', '4.8★', 'Bandra West', 'interior designers'];

  console.log('================================================================================');
  console.log(`🎯 DISPATCHING TO TEST NUMBER: +91${targetPhone}`);
  console.log(`🏷️ Template: "${templateName}"`);
  console.log(`📌 Variables: ${JSON.stringify(params)}`);
  console.log(`📡 VIA CRM API KEY: ak_live_bb3a202dc4c32629a10ebb3a2c3f86a4`);
  console.log('================================================================================\n');
  ...
  try {
    ...
    if (res.success) {
      console.log(`\n✅ SUCCESSFULLY DISPATCHED TO ${targetPhone}! Message ID: ${res.messageId}`);
    } else {
